    ...
    public class InfoSheetTests : BaseClass
    {
		...
		public void ReassignMatter()
		{
			try
			{
				...
				// Edit secretary
				test.Log(Status.Info, $"[{testClassName}] Reassigning matter");
				InfoSheetPage.ReassignSecretary(TestData.RedirectMatterTo);
				test.Log(Status.Pass, $"[{testClassName}] Successfully reassigned matter");
